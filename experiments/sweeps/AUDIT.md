...
| 56 | sweep 026 | `--drain-steps` was added to `binding.py` with a default of **200**, and the four standing conditions are invoked without the flag, so every condition silently gained a 200-step silent tail | **sweep 026's first run only**, which is re-run and recorded. The baseline moved 0.802 → 0.620, the largest single baseline movement in the project. No earlier sweep is touched — `binding.py` never passed `drain_steps` before this, so every prior run used 0. Third instance of "a new flag's default is a silent change to every existing condition" (sweep 003's `PRETRAIN`, sweep 013's drain tail); the default is corrected to 0 and conditions that want a tail pass it explicitly. |
| 57 | sweep 028 | the offline `+0.074` was quoted as a property of the column, but the decoder's collection budget is part of it | **nothing is void; every offline binding number now needs its budget attached.** At 500 collection episodes the gain is +0.074 (0.802 → 0.876); at 1200 it is +0.060 (0.853 → 0.913). The difference is the share of the gain that comes from binding making the representation *cheaper to fit* rather than better, and it shrinks as the decoder stops being starved. Both measurements are correct; neither transfers to a differently-sized probe. |
...
| Learning does **not** survive a late modulator; the window is 12 steps | sweep 026, `lagwindow.py` | 20 paired seeds, plus a direct probe at three parameter settings | Binding gives +0.127 at lag 0 (19/20, p = 0.0000) and is null from lag 25 on. The window is set by the *product* of `act_fast` (τ 50) and the branch filter (τ 15), giving τ_eff = 11.5; measured 11.87 / 15.25 / 28.68 against predictions of 11.54 / 14.15 / 27.27. Frozen column, `lr=0`, so no learning-path fix can reach it. **This bounds the architecture's headline claim rather than supporting it**: inference across columns tolerates 150 ms, the modulator path tolerates 12. |
| The settling operating point's threshold half is computable at construction | sweep 029, theta probe | 20 paired seeds, plus a 6-seed fit | `theta = 5.927 * tau^-0.748` (log-log r = -0.974) applied at construction reaches 0.757, null against a settled twin's thresholds at p = 0.7672, where ordinary settling starts at 0.582 and takes 100 episodes to reach 0.755. Frozen column, `lr = 0`, so no learning-path fix can reach it. **The knee half is NOT computable** -- `tau-init-knee` still copies a twin's knee, and `preset-knee` alone sits at 0.566 with sparsity 0.002. And the exponent is fitted at 96 neurons on one task, never shown universal. |
| The settling operating point is a per-neuron fit, not a recipe | sweep 027 | 20 paired seeds, p = 0.0000 | Permuting theta and the knee between neurons -- distribution preserved exactly, ownership destroyed -- costs 0.130 on 0/20 seeds, and the permuted column fires at double target. This is what made "a fit to what?" the live question that sweep 029 answered. Frozen column throughout. |
| The routed vector modulator is inert in every recorded configuration | `test_the_routed_modulator_carries_no_information_in_a_working_column` | direct test, both directions | DESIGN.md calls it "probably the largest single lever on credit-assignment quality". Its content reaches the weights only through `W += cfg.lr * signal * drive`, and every sweep from 014 onward runs `lr = 0`; binding reads the modulator for *presence* only. Scrambling the content leaves weights bit-identical at `lr = 0` and changes them at `lr = 0.004`. **Also: the default `feedback_mode="symmetric"` is weight transport, not the per-neuron random projection the document describes** -- `dfa` is that, and the two have never been compared at 20 seeds. Under the default, DESIGN.md's bandwidth claim is false (readout matrix is `n_neurons x n_classes`). |
| The fast gather path agrees bit-for-bit with the checked one | `test_the_fast_gather_path_agrees_with_the_checked_one` | direct test | `LocalTransport` sets `dense = True`, so **every experiment, sweep and README number in this project was computed through `take`** -- the unchecked flat-index read -- while `gather`, which resolves (source, delay) and masks slots not holding the requested emission time, is the correct one. Nothing asserted they agree; no test even referenced `take`. They do, bit-identically over three training episodes on output and weights. A divergence would not have raised, it would have meant every recorded result described a model nobody intended. |
| The standing benchmark is **not** short of headroom for a learning rule | `experiments/headroom.py` | 3 seeds, frozen column, 10 configurations | Claimed in TODO item 11 that delayed XOR leaves a learning rule almost nothing to win. **Refuted.** The linear-to-MLP gap is 0.214 at the grid's settings and 0.185 at the standing ones (linear 0.802 vs MLP 0.987) -- the label is nearly fully *present* but NOT nearly fully linearly accessible. Binding's +0.074 captures 40% of that gap and lateral's +0.028 another 15%, so the mechanisms are taking a substantial share rather than being starved. No tested configuration beats it: noise, weak cues, longer gaps and a third cue all destroy the information rather than tangling it. **Caveat: the gap is measured with a 64-unit MLP, so it is an upper bound on what a local rule could win, not a promise.** Residual value: the memory wall is now mapped -- cues 100 steps apart survive, 200 do not (gap -0.003), three cues do not at any size. |
| The model is **not** memory-bandwidth-bound; it is overhead-bound | `experiments/roofline.py` | direct measurement, this machine | 17.1 % of measured sustained DRAM bandwidth at 96 neurons, rising to 29.3 % at 1024 -- utilisation *rising* with size is per-call overhead amortising, not bandwidth saturating. The whole 8-array `(N,B,S)` working set is 384 KB at 96 neurons against an 8 MiB L2, so most of even that traffic is cache. **Refutes the written claim in TODO item 8**, and with it the framing of the GPU counter-argument: the comparison cannot be made while the implementation is nowhere near any hardware limit. Also establishes ~450-770 neurons per core at biological real time. Machine-specific by nature; re-run `roofline.py` anywhere the number is quoted. |
| Delivery jitter below `delay_min` leaves a distributed run bit-identical | `test_delivery_jitter_does_not_change_a_distributed_run` | direct test, both directions asserted | The architecture's headline claim, previously tested only at the primitive level (one buffer, two out-of-order events). A three-column run with every published slice delayed 0-N steps and applied in shuffled order matches a punctual one exactly below `delay_min`, and differs above it. Tolerance is `delay_min - 1` steps and no more. Unaffected by any sweep or learning-path fix -- it is an assertion, re-checked on every run. |
| Learning survives *regional* modulator lag, bought with `tau_branch` | sweep 030 | 20 paired seeds | At `tau_branch 60`, binding pays +0.062 at lag 25 (20/20, p = 0.0000) and +0.037 at lag 50 (17/20, p = 0.0000) against a reference at the same `tau_branch`. At the default the lag-25 comparison is null. Frozen column, `lr = 0`. **Does not reach lag 150**, and `on-tb120-lag50`'s null is uninformative because its reference sits at 0.918 with no headroom. |
| `bind_tau_pre` does NOT buy latency tolerance | sweep 030 | 20 paired seeds | The 154-step window is real in the direct probe and does not pay: lag-150 gain +0.019 at p = 0.0794 (null), and -0.099 at lag 0 on 0/20 seeds at p = 0.0000. A window that spans the episode destroys the temporal specificity the mechanism runs on. Flag ships off; the measurement is kept, the description must not say "latency tolerance". |
| `tau_branch 60` is worth **+0.056**, not +0.298 -- the rest was the drain tail | sweeps 030 and 034 | 20 paired seeds each | 030 measured +0.298 at `drain_steps 200`; 034 removed the tail and got **+0.056 at p = 0.0004** (best setting, `tau_branch 60`), so roughly four fifths of the largest single-parameter effect in the project was compensation for 200 silent steps. Both link conditions reproduced 030 exactly, so this re-scopes rather than contradicts. The curve turns over at `drain 0` -- interior optimum at 60, declining through 120 and 240 -- where under the tail it was monotone. **This is the entry that was flagged at the time as "a result about a configuration", and the guard worked.** `tau_branch = 60` is now a known better setting the defaults do not use, held back deliberately: every standing number was measured at 15, so re-baselining is a decision to take on purpose with its invalidations listed first. |
| The binding gain is size-independent from 48 to 192 neurons | sweep 028 | 20 paired seeds per size | +0.078 / +0.060 / +0.066, all p ≤ 0.0001, with fan-in fixed so per-neuron computation is identical at every size. The `48-thin` control (decoder starved to the largest column's samples-per-feature) is **null at p = 0.0684**, which is what rules out decoder overfitting as the source. Frozen column throughout. Does **not** extend above 192: `off-384` reaches 0.973 and the task, not the column, is what saturates. |
