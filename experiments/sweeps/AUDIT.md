...
| `tau_branch 60/120` is worth +0.218/+0.298 -- **AT `drain_steps 200` ONLY** | sweep 030 | 20 paired seeds, p = 0.0000 | Larger than settling's +0.197 and than every learning rule combined, found as a *control* for something else. **Not established as a general result.** Every condition ran at `drain_steps 200` where the baseline is 0.620 against the tail-free 0.802, so `tau_branch` may be compensating for the 200 silent steps rather than improving the column. Quoting +0.298 as general would repeat sweep 026's baseline-shift error exactly. Sweep 034 sweeps it at `drain_steps 0`; until then this is a result about a configuration. |
