...
| item | how | status |
|---|---|---|
| Does lateral inhibition pay end to end? | sweep 021 | **the live question.** Sweep 020 measured it offline only, at +0.037 on top of binding, and sweep 016 is the standing warning that offline gains need not arrive: binding's +0.074 came through as +0.007. `latbind-on` against `rls-on` decides whether a local rule reaches what pooling reaches |
| What lateral inhibition is actually doing | new probe | Built to decorrelate; sweep 020 measured it not decorrelating while helping. The mechanism is kept on the measurement and its explanation is **open** |
